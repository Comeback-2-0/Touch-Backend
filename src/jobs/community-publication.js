...
const {scheduleCommunityTrendingRecompute} = require('../modules/communities/community-trending');
...
async function runCommunityPublication() {
  ...
  for (const item of published) scheduleCommunityTrendingRecompute(item.communityId);
  return published;
}
